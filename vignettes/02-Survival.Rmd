...
Here we describe the parameters associated with survival.
...
Where $c_{i, 1}(t + 1)$ is defined by based on a binominal distribution with $c_{i,1}(t)$ trials and probability of success equal to $S_{i,1}(t)$. As aii increases, the number of individuals expected to survive decreases (**Figure 1**).   


```{r immature, echo=FALSE, fig.align='center' }
...
```
**Figure 1**: Expected number of individuals to survive when the immature survival probability is equal to the exponetial of aii times the population size.
...
We can define $\alpha$ and $\beta$ based on $\mu$ and $env.ci$ as followed: 
...
```{r mature, echo=FALSE, fig.align='center'}
