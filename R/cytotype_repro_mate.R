...
cytotype_repro_mate <- function(cgen, b, cc, gnum.vec, s, mc) {
  ...
  if (ngd == 0 & ngt == 0 & nga == 0) {
    ...
    d1nself <- floor(D1n*prob1nself); if (is.na(d1nself)) {d1nself <- 0} # 1n selfing gametes
    d2nself <- floor(D2n*prob2nself); if (is.na(d2nself)) {d2nself <- 0} # 2n selfing gametes
    ...
    ## Assuming selfing only occurs with like-gamete types (ie. 1n x 1n, 2n x 2n)
    ...
    if (mc == 0) {
      ...
      if (totaloutcrossing != 0) {
          ...
          ## Probability of choosing two from the same individual
          ### Diploid
          if(ndi > 0){
              pnd <- matrix(nrow = ndi, ncol = 2)
            if(d1nout > 0 & d2nout > 0){
                pnd[,1] <- rmultinom(n = 1, size = d1nout, prob = rep(1/ndi, ndi))[,1]
                pnd[,2] <- rmultinom(n = 1, size = d2nout, prob = rep(1/ndi, ndi))[,1]
                tots <- c(d1nout, d2nout)
                tots.offs <- apply(pnd, 1, sum)
                prob1ndsame <- sum(na.omit(dhyper(x = 2, m = pnd[,1], n = tots[1] - pnd[, 1], k = 2)));  if (is.na(prob1ndsame)) {prob1ndsame <- 0}
                prob2ndsame <- sum(na.omit(dhyper(x = 2, m = pnd[,2], n = tots[2] - pnd[, 2], k = 2)));  if (is.na(prob2ndsame)) {prob2ndsame <- 0}

                xa <-dhyper(x = 2, m = tots.offs, n = sum(tots) - tots.offs, k = 2)
                xa[is.na(xa)] <- 0
                xb <- dhyper(x = 1, m = pnd[,1], n = tots.offs - pnd[,1], k = 2)
                xb[is.na(xb)] <- 0
                prob1n2ndsame <- sum(na.omit(xa*xb));  if (is.na(prob1n2ndsame)) {prob1n2ndsame <- 0}
            } else if(d1nout > 0 & d2nout == 0){
                pnd[,1] <- rmultinom(n = 1, size = d1nout, prob = rep(1/ndi, ndi))[,1]
                pnd[,2] <- 0
                tots <- apply(pnd, 2, sum)
                tots.offs <- apply(pnd, 1, sum)
                prob1ndsame <- sum(na.omit(dhyper(x = 2, m = pnd[,1], n = tots[1] - pnd[, 1], k = 2)));  if (is.na(prob1ndsame)) {prob1ndsame <- 0}
                prob2ndsame <- 0
                prob1n2ndsame <- 0
          ...
          ## Triploid
          if (t3nout > 0) {
            ptd <- rmultinom(n = 1, size = t3nout, prob = rep(1/nti, nti))
            prob3ntsame <- sum(na.omit(dhyper(x = 2, m = ptd[,1], n = t3nout-ptd[, 1], k = 2))); if (is.na(prob3ntsame)) {prob3ntsame <- 0}
          } else {
            prob3ntsame <- 0
          }
          ...
          ## Tetraploid
          if(a2nout > 0){
            pna <- rmultinom(n = 1, size = a2nout, prob = rep(1/nai,nai))
            prob2nasame <- sum(na.omit(dhyper(x = 2, m = pna[,1], n = a2nout-pna[, 1], k = 2))); if (is.na(prob2nasame)) {prob2nasame <- 0}
          } else {
            prob2nasame <- 0
          }
          ...
          mates.vec[is.na(mates.vec)] <- 0
            ...
            if (sum(mates.vec) != 0) {
              ...
              ## Correct for unsuccessful offspring
              out.t[1] <- rbinom(n = 1, size = out.t[1], prob = (1 -  prob1ndsame))
              out.t[2] <- rbinom(n = 1, size = out.t[2], prob = (1 -  prob1n2ndsame))
              out.t[5] <- rbinom(n = 1, size = out.t[5], prob = (1 -  prob1n2ndsame))
              out.t[6] <- rbinom(n = 1, size = out.t[6], prob = (1 -  prob2ndsame))
              out.t[16] <- rbinom(n = 1, size = out.t[16], prob = (1 - prob2nasame))

              ## Count successful offspring
    ...
    ## (2) Choosy mating. ie. assortative mating
      ...
      if (tot2x != 0) {
        ...
        ### Diploid
        if(ndi > 0){
        pool2x_pnd <- matrix(nrow = ndi, ncol = 2)
        if(pool2x_1n > 0 & pool2x_2n > 0){
            pool2x_pnd[,1] <- rmultinom(n = 1, size = pool2x_1n, prob = rep(1/ndi, ndi))[,1]
            pool2x_pnd[,2] <- rmultinom(n = 1, size =  pool2x_2n, prob = rep(1/ndi, ndi))[,1]
            pool2x_tots <- apply(pool2x_pnd, 2, sum)
            pool2x_tots.offs <- apply(pool2x_pnd, 1, sum)
            pool2x_prob1ndsame <- sum(na.omit(dhyper(x = 2, m = pool2x_pnd[,1], n = pool2x_tots[1]-pool2x_pnd[, 1], k = 2))); if (is.na(pool2x_prob1ndsame)) {pool2x_prob1ndsame <- 0}
            pool2x_prob2ndsame <- sum(na.omit(dhyper(x = 2, m = pool2x_pnd[,2], n = pool2x_tots[2]-pool2x_pnd[, 2], k = 2))); if (is.na(pool2x_prob2ndsame)) {pool2x_prob2ndsame <- 0}
            pool2x_xa <- dhyper(x = 2, m =  pool2x_tots.offs, n = sum(pool2x_tots), k = 2)
            pool2x_xa[is.na(pool2x_xa)] <- 0
            pool2x_xb <- dhyper(x = 1, m =  pool2x_pnd[,1], n = (pool2x_tots.offs - pool2x_pnd[,1]), k = 2)
            pool2x_xb[is.na( pool2x_xb)] <- 0
            pool2x_prob1n2ndsame <- sum(na.omit(pool2x_xa*pool2x_xb));  if (is.na(pool2x_prob1n2ndsame)) {pool2x_prob1n2ndsame <- 0}
        } else if(pool2x_1n > 0 & pool2x_2n == 0){
            pool2x_pnd[,1] <- rmultinom(n = 1, size = pool2x_1n, prob = rep(1/ndi, ndi))[,1]
            pool2x_pnd[,2] <- 0
            pool2x_tots <- apply(pool2x_pnd, 2, sum)
            pool2x_tots.offs <- apply(pool2x_pnd, 1, sum)
            pool2x_prob1ndsame <- sum(na.omit(dhyper(x = 2, m = pool2x_pnd[,1], n = pool2x_tots[1]-pool2x_pnd[, 1], k = 2))); if (is.na(pool2x_prob1ndsame)) {pool2x_prob1ndsame <- 0}
            pool2x_prob2ndsame <- 0
            pool2x_prob1n2ndsame <- 0
        } else if(pool2x_1n == 0 & pool2x_2n > 0){
            pool2x_pnd[,1] <- 0
            pool2x_pnd[,2] <- rmultinom(n = 1, size = pool2x_2n, prob = rep(1/ndi, ndi))[,1]
            pool2x_tots <- apply(pool2x_pnd, 2, sum)
            pool2x_tots.offs <- apply(pool2x_pnd, 1, sum)
            pool2x_prob1ndsame <- 0
            pool2x_prob2ndsame <- sum(na.omit(dhyper(x = 2, m = pool2x_pnd[,2], n = pool2x_tots[2]-pool2x_pnd[, 2], k = 2))); if (is.na(pool2x_prob2ndsame)) {pool2x_prob2ndsame <- 0}
            pool2x_prob1n2ndsame <- 0
        } else {
            pool2x_prob1ndsame <- 0
        ...
        if (sum(p2x_mates) != 0) {
            ...
            ## Correct for unsuccessful offspring
            p2x_out.t[1] <- rbinom(n = 1, size =  p2x_out.t[1], prob = (1 - pool2x_prob1ndsame))
            p2x_out.t[2] <- rbinom(n = 1, size =  p2x_out.t[2], prob = (1 - pool2x_prob1n2ndsame))
            p2x_out.t[3] <- rbinom(n = 1, size =  p2x_out.t[3], prob = (1 - pool2x_prob1n2ndsame))
            p2x_out.t[4] <- rbinom(n = 1, size =  p2x_out.t[4], prob = (1 - pool2x_prob2ndsame))
      ...
      ## Correct for unsuccessful offspring
      if(pool4x > 0){
        pool4x_pna <- rmultinom(n = 1, size = pool4x , prob = rep(1/nai, nai))
        pool4x_prob2nasame <- sum(dhyper(x = 2, m = pool4x_pna[,1], n = pool4x-pool4x_pna[, 1], k = 2)); if (is.na(pool4x_prob2nasame)) {pool4x_prob2nasame <- 0}
        p4x_4x <- rbinom(n = 1, size = p4x_4x, prob = (1 - pool4x_prob2nasame))
      } else {
        p4x_4x <- p4x_4x
      }
      ...
      total_poolrm <- sum(na.omit(c(poolrm_1nd, poolrm_2nd, poolrm_3nt, poolrm_2na))) ; if (is.na(total_poolrm)) {total_poolrm <- 0}
       if (total_poolrm != 0) {
        ...
        ### Diploid
        if(ndi > 0){
            poolrm_pnd <- matrix(nrow = ndi, ncol = 2)
          if(poolrm_1nd > 0 &  poolrm_2nd > 0){
            poolrm_pnd[,1] <- rmultinom(n = 1, size = poolrm_1nd, prob = rep(1/ndi, ndi))[,1]
            poolrm_pnd[,2] <- rmultinom(n = 1, size = poolrm_2nd, prob = rep(1/ndi, ndi))[,1]

            poolrm_tots <- apply(poolrm_pnd, 2, sum)
            poolrm_tots.offs <- apply(poolrm_pnd, 1, sum)
            poolrm_prob1ndsame <- sum(na.omit(dhyper(x = 2, m = poolrm_pnd[,1], n = poolrm_tots[1]-poolrm_pnd[, 1], k = 2))); if (is.na(poolrm_prob1ndsame)) {poolrm_prob1ndsame <- 0}
            poolrm_prob2ndsame <- sum(na.omit(dhyper(x = 2, m = poolrm_pnd[,2], n = poolrm_tots[2]-poolrm_pnd[, 2], k = 2))); if (is.na(poolrm_prob2ndsame)) {poolrm_prob2ndsame <- 0}

            poolrm_xa <- dhyper(x = 2, m = poolrm_tots.offs, n = sum(poolrm_tots) - poolrm_tots.offs, k = 2)
            poolrm_xa[is.na(poolrm_xa)] <- 0
            poolrm_xb <- c()
            poolrm_xb <- dhyper(x = 1, m = poolrm_pnd[,1], n = (poolrm_tots.offs - poolrm_pnd[,1]), k = 2)
            poolrm_xb[is.na(poolrm_xb)] <- 0
            poolrm_prob1n2ndsame <- sum(na.omit(poolrm_xa*poolrm_xb));  if (is.na(poolrm_prob1n2ndsame)) {poolrm_prob1n2ndsame <- 0}
          } else if (poolrm_1nd > 0 &  poolrm_2nd  == 0){
            poolrm_pnd[,1] <- rmultinom(n = 1, size = poolrm_1nd, prob = rep(1/ndi, ndi))[,1]
            poolrm_pnd[,2] <- 0
            poolrm_tots <- apply(poolrm_pnd, 2, sum)
            poolrm_tots.offs <- apply(poolrm_pnd, 1, sum)
            poolrm_prob1ndsame <- sum(na.omit(dhyper(x = 2, m = poolrm_pnd[,1], n = poolrm_tots[1]-poolrm_pnd[, 1], k = 2))); if (is.na(poolrm_prob1ndsame)) {poolrm_prob1ndsame <- 0}
            poolrm_prob2ndsame <- 0

            poolrm_prob1n2ndsame <- 0
          } else if (poolrm_1nd == 0 &  poolrm_2nd  > 0){
            poolrm_pnd[,1] <- 0
            poolrm_pnd[,2] <- rmultinom(n = 1, size = poolrm_2nd, prob = rep(1/ndi, ndi))[,1]
            poolrm_tots <- apply(poolrm_pnd, 2, sum)
            poolrm_tots.offs <- apply(poolrm_pnd, 1, sum)
            poolrm_prob1ndsame <- 0
            poolrm_prob2ndsame <- sum(na.omit(dhyper(x = 2, m = poolrm_pnd[,2], n = poolrm_tots[2]-poolrm_pnd[, 2], k = 2))); if (is.na(poolrm_prob2ndsame)) {poolrm_prob2ndsame <- 0}
            poolrm_prob1n2ndsame <- 0
          } else {
            poolrm_prob1ndsame <- 0
        ...
        ## Triploid
        if( poolrm_3nt > 0){
          poolrm_ptd <- rmultinom(n = 1, size = poolrm_3nt, prob = rep(1/nti, nti))
          poolrm_prob3ntsame <- sum(na.omit(dhyper(x = 2, m = poolrm_ptd[,1], n = poolrm_3nt-poolrm_ptd[, 1], k = 2))); if (is.na(poolrm_prob3ntsame)) {poolrm_prob3ntsame <- 0}
        } else {
          poolrm_prob3ntsame <- 0
        }
        ## Tetraploid
        if(poolrm_2na > 0){
        poolrm_pna <- rmultinom(n = 1, size = poolrm_2na, prob = rep(1/nai,nai))
        poolrm_prob2nasame <- sum(na.omit(dhyper(x = 2, m = poolrm_pna[,1], n = poolrm_2na-poolrm_pna[, 1], k = 2))); if (is.na(poolrm_prob2nasame)) {poolrm_prob2nasame <- 0}
        } else {
          poolrm_prob2nasame <-0
        }
        ...
        mates.vec[is.na(mates.vec)] <- 0
          ...
          if (sum(mates.vec) != 0) {
            ...
            ## Correct for unsuccessful offspring
            out.t[1] <- rbinom(n = 1, size = out.t[1], prob = (1 - poolrm_prob1ndsame))
            out.t[2] <- rbinom(n = 1, size = out.t[2], prob = (1 -   poolrm_prob1n2ndsame))
            out.t[5] <- rbinom(n = 1, size = out.t[5], prob = (1 -   poolrm_prob1n2ndsame))
            out.t[6] <- rbinom(n = 1, size = out.t[6], prob = (1 -   poolrm_prob2ndsame))
            out.t[16] <- rbinom(n = 1, size = out.t[16], prob = (1 -  poolrm_prob2nasame))
